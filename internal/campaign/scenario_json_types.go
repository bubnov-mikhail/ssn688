...
// JSON document types for portable scenario files (format major 3).
// Player-facing prose uses LocText objects {"en":"...","ru":"..."}.
...
type scenarioFile struct {
	...
	Backstory         LocText        `json:"backstory"`
	Cover             *assetBlobJSON `json:"cover,omitempty"`
	PostscriptSuccess LocText        `json:"postscript_success,omitempty"`
	PostscriptFailure LocText        `json:"postscript_failure,omitempty"`
...
type missionJSON struct {
	...
	CommBriefing LocText           `json:"comm_briefing,omitempty"`
...
type debriefLineJSON struct {
	...
	OnSuccess   LocText `json:"on_success"`
	OnFail      LocText `json:"on_fail"`
}

type commMsgJSON struct {
	ID    string  `json:"id"`
	AtSec float64 `json:"at_sec"`
	Text  LocText `json:"text"`
}
